...
const TokenAPI = require("./util/token.js");
const token=TokenAPI.Decoder
const ws = require("ws");
...
function CorrectEventName(event_name) {
  switch(event_name) {
    case "podium":
      return "Podium"
    case "question":
...
class Client extends EventEmitter{
   ...
   * @param {(...args:any[])=>void} callback The event handler
   */
   on(eventName,callback) {
    super.on(
      CorrectEventName(eventName),
      callback
    )
  }
  /**
   * Listens for an event, alias for `on`
   * @param {string|symbol} eventName The event name
   * @param {(...args:any[])=>void} callback The event handler
   */
      ...
      this.handlers.JoinFinish = async (message)=>{
        if(message.channel === "/service/status"){
          ...
          if(message.data.status === "LOCKED"){
            ...
            this.emit("locked",{
              toString:()=>"[object QuizLockedEvent]"
            })
            this.leave(true);
            return;
          }
        ...
        if(message.channel === "/service/controller" && message.data && message.data.type === "loginResponse"){
          if(message.data.error){
            ...
            if(message.data.description.toLowerCase() !== "duplicate name"){
              ...
              this.emit("samename",{
                toString:()=>["object DuplicateNameEvent"]
              })
              this.leave(true);
            }
          }else{
    ...
    return new Promise((resolve,reject)=>{
      this._send(new this.classes.LiveJoinTeamPacket(this,team),(r)=>{
        if(r === null || !r.successful){
          ...
          !s && this.emit("connect",this.settings);
          if(!this.settings.twoFactorAuth){
            this.connected = true;
          }else{
  ...
  async _createHandshake(){
    ...
    return new Promise((res,rej)=>{
      ...
      this.socket.on("message",(message)=>{
        this._message(message.toString('utf-8'));
      });
